...
def generate_topic_lvl1(
    ...
    Parameters:
    - api (str): API to use ('openai', 'vertex', 'vllm', 'azure', 'gemini', 'openrouter')
    - model (str): Model to use (e.g. 'meta-llama/llama-3.1-405b-instruct')
    - data (str): Data file (.jsonl)
    - prompt_file (str): File to read prompts from
    - seed_file (str): Markdown file to read seed topics from
    - out_file (str): File to write per-document responses to
    ...
    # Model configuration (128k for non-GPT-3.5/4, which includes Llama 3.1)
    context = (
        128000
